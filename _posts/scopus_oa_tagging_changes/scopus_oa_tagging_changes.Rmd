...
Figure \@ref(fig:scpsankey) shows the flows between the snapshots, i.e. the number of mapped tags between snapshots.
...
Nodes titled *Missing* indicate cases where the number of tags varied between snapshots for the same item.
...
They show that for the sample of `r n_distinct(green_none_0407$doi)` items that lost their green status between the April and July snapshots, Unpaywall found a repository copy, i.e. a green open access version, for `r nrow(filter(green_none_0407, has_repository_copy == TRUE) %>% distinct(doi))` (`r round(nrow(filter(green_none_0407, has_repository_copy == TRUE) %>% distinct(doi)) / nrow(distinct(green_none_0407, doi)) * 100, 3)`%) of them. 
Furthermore, Unpaywall assigned green as primary open access status to `r nrow(filter(green_none_0407, host_type == "repository" & oa_status == "green") %>% distinct(doi))` (`r round(nrow(filter(green_none_0407, oa_status == "green" & host_type == "repository") %>% distinct(doi)) / nrow(filter(green_none_0407, has_repository_copy == TRUE) %>% distinct(doi)) * 100,2)`%) of the items with a repository copy. 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
load(file=here("_posts/scopus_oa_tagging_changes/data","upw_0407_recopy.RData"))
load(file=here("_posts/scopus_oa_tagging_changes/data","upw_0407_host.RData"))
```
...
cat("<div style='text-align: center; font-style: italic; font-size: 0.8em; margin-top: 10px;'>Unpaywall OA information April 24 and July 24 comparison</div>")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
load(file=here("_posts/scopus_oa_tagging_changes/data","upw_0710_recopy.RData"))
load(file=here("_posts/scopus_oa_tagging_changes/data","upw_0710_host.RData"))
```
...
cat("<div style='text-align: center; font-style: italic; font-size: 0.8em; margin-top: 10px;'>Unpaywall OA information July 24 and October 24 comparison</div>")
```
...
The analysis indicates no difference between the Scopus raw data delivery and the data in Scopus' own database.
