...
scp_oa_share <- scp_sc_prep %>%
 ...
 mutate(
 total = sum(n),
 share = format(n / total * 100, nsmall = 2, decimal.mark = ".")
 ) %>%
 select(snapshot, is_oa, n, share) %>%
 mutate(snapshot = ifelse(snapshot =="scp04_is_oa", "April",
                          ifelse(snapshot == "scp07_is_oa", "July",
                                 ifelse(snapshot == "scp10_is_oa","October",snapshot))))
scp_oa_share
```
