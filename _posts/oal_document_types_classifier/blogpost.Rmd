...
But there are also other types, such as letters to the editor or book reviews, which are often excluded.

Both the vocabulary used to describe journal article types and the methods used to assign these categories vary between bibliometric databases. 
For example, when [analysing the classification of journal article types in OpenAlex, Web of Science (WoS), Scopus, PubMed and Semantic Scholar](https://arxiv.org/abs/2406.15154), I found that OpenAlex tends to overestimate the assignment of the document type <i>article</i>. OpenAlex tagged 10% of items as articles, which were labelled as editorial material in Scopus and the Web of Science.
...
In addition, articles from publishers with fewer than 5,000 publications were excluded from the dataset. 
...
## Results
Overall, my classifier identifies 12.647.946 (11.6%) of 108.744.219 journal items classified as articles and reviews in OpenAlex as non-research items.
When restricting to the period 2012 to 2021, this figure drops to 3.778.393 (9.9%) out of 38.265.399 journal items, which can be explained by a lack of metadata for journal articles published before 2012.

Figure \@ref(fig:cwts) compares the results of my classifier with OpenAlex and the CWTS, based on items in scientific journals between 2012 and 2021. The left side of the figure displays all journals in OpenAlex whereas the data from the figure on the right side is restricted to CWTS core journals. The grey line shows all articles and reviews in journals in OpenAlex. The green line shows all articles and reviews in OpenAlex for which at least one reference and one citation was found in OpenAlex. The purple line below shows all items from journals in OpenAlex that are included as core publications in the CWTS Leiden Ranking Open Edition. The yellow line shows the proportion of items in OpenAlex that are recognised as research items by my classifier. 

About 48.3% of all journal items in OpenAlex from 2012 to 2021 were categorised as core publications by the CWTS when not restricting to core journals. This figure is probably due to the exclusion of journals that do not fulfil the criteria for CWTS core journals[@van_eck_2024_13879947]. Only 27.721 (13.6%) of all 203.545 journals in OpenAlex are considered as core journals by the CWTS. Also, missing authors, affiliations and references have an impact on the CWTS classification. In contrast, my classifier identifies about 84,5% of journal items as research items (when not restricting to core journals) which may demonstrate that the classifier is less sensitive to missing metadata or a low number of citations. This observation can be also made when comparing the green and yellow lines. If you look at the right-hand side of the figure, in which I restrict the data to the core journals, you can see that both the proportion of core publications and the proportion of publications with at least one reference and one citation are increasing. Notably, they achieve both about 71% on average. 
...
To check for potentially discriminatory behaviour of my classifier, I compared my approach with the CWTS approach by journal subject (see Figure \@ref(fig:topics)). Again, a common corpus based on DOI matching is used. Figure \@ref(fig:topics)a) and \@ref(fig:topics)b) displays all journals in OpenAlex whereas the data in Figure \@ref(fig:topics)c) and Figure \@ref(fig:topics)d) is limited to CWTS core journals.
Figure \@ref(fig:topics)a) and \@ref(fig:topics)b) shows that my classifier treats publications from different disciplines in a more balanced way, while the CWTS method excludes publications from the social sciences and humanities more often. Again, this could be due the exclusion of non-core journals. Here, [the CWTS](https://www.leidenranking.com/information/indicators) points out that many social science journals were not considered as core journals due to the lack of a suitable number of references. A comparison of Figure \@ref(fig:topics)c) and Figure \@ref(fig:topics)d) shows that this behaviour is changing. Here, my classifier and the CWTS classification show a similar pattern, with publications from the health sciences being excluded more frequently. These publications may be case reports, which usually contain no references.
