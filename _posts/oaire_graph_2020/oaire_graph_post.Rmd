...
The function `openairegraph::oarg_decode()`  splits and de-codes each record in a OpenAIRE Research Graph file. Storing the records individually allows to process the files independent from each other, which is a common approach when working with big data. 
...
After that, I use the `future`[@future] and `future.apply `[@future_apply] packages to enable reading and parsing these records simultaneously with multiple R sessions. Running code in parallel reduces the execution time.

In the following, I read each XML file using the `xml2`[@xml2] package, and apply  three parsers: `openairegraph::oarg_publications_md()`, `openairegraph::oarg_linked_projects()` and `openairegraph::oarg_linked_ftxt()`.
...
It contains `r oaire_df %>% distinct(id) %>% nrow()` grant-supported research outputs. In this use case, I will focus on the prevalence of open access across H2020 projects using metadata about the open access status of a publication and related project information stored in the list-column `linked_projects`.
...
```{r, fig.cap="Publication Output of Horizon 2020 funding activities captured by the OpenAIRE Research Graph, released in December 2019."}
library(cowplot)
library(scales)
pubs_projects %>%
...
The resulting data, `oa_monitor_ec`, allows to explore variations among and within H2020 funding programmes. 

```{r, fig.cap="Open Access Compliance Rates of Horizon 2020 projects relative to funding activities, visualised as box plot. Only projects with at least five publications were considered."}
oa_monitor_ec %>%
...
About 77% of research pubilcations under the H2020 open access mandate are openly available. The Figure highlights a generally high rate of compliance with the open access mandate. However, rates vary among and within the H2020 funding programme. 
...
Next, I identify H2020 projects with participation from the university. There at least two ways to obtain links between projects and organisations: One is the OpenAIRE Research Graph. It provides project details from 29 funder in a separate dump, `project.gz`. Another option is to relate our dataset to [open data provided by the European Commission](https://data.europa.eu/euodp/en/data/dataset/cordisH2020projects). For convenience, I am going to follow the second option.
...
After loading the file, I identify projects affiliated with the University of Göttingen.
...
Let's put it all together and benchmark the rates of compliance with the open access mandate using data from the OpenAIRE Research Graph. The package `plotly`[@plotly] allows presenting the figure as interactive chart.
