...
The OpenAIRE Research Graph provides a wide range of metadata about scholarly communication that includes scholarly publications interlinked with persons, organisations and funding streams. Since December 2019, this data can be accessed as a dump. 

While such data dump offers many opportunities for data analyses, working with them is often laborious, especially when tools for importing and transforming data of such size on a regular basis are not available for your statistical computing environment. In this blog post, I present openairegraphs, a R package that helps to load and transform data from the OpenAIRE Research Graph following tidy data principles. Focusing on grant-supported research results from the European Commission's Horizon 2020 funding programme, I present some general data characteristics, followed by a real-world example showcasing the usefulness of the OpenAIRE Research Graph for monitoring the uptake of the European Commission's open science policy.

## What is the R package openairegraphs about?

So far, the R package openairegraphs, which is available on GitHub, contains two sets of functions. The first set provides helpers to split and de-compress a large OpenAIRE Research Graph data file into seperate XML records that can be stored in a dedicated folder on your local computer. The other set consists of parsers that convert data from these XML files to rectangular data frames. This is an essential step before analysing the OpenAIRE Research Graph with R.
...
Several dumps from the OpenAIRE Research Graph are available on Zenodo. `openairegraph` has been tested to work with the dump `h2020_results.gz`, which comprises research outputs funded by the European Commission's Horizon 2020 funding programme.

After downloading it, the dump `h2020_results.gz` can be imported into R using the jsonlite package. In the json file, each line contains the record identifier and the corresponding Base64-encoded XML record. Base64 is a standard that allows file compression in a text-based format.
