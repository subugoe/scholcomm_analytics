...
oal_scp_sn <- sankeyNetwork(Links = links, Nodes = nodes, Source = "source",
              Target = "target", Value = "value", NodeID = "label", fontSize = 12, nodePadding = 20, 
                    zoom = TRUE, showNodeValues = FALSE, numberFormat = ",d", fontFamily = "sans-serif", iterations=0,NodeGroup="group", colourScale = colours,dragX = TRUE,dragY = TRUE, xAxisDomain = c("OpenAlex (2023)","OpenAlex (2024)","Web of Science (2024)"),xScalingFactor = .7, height=700)
...
oal_scp_sn <- onRender(
  ...
  function(el,x){
    ...
    // We now use the tested and saved conditions
    ...
    if(zoomLayer){
      zoomLayer.setAttribute("transform", "translate(198,72) scale(0.75)");
    }
...
When comparing OpenAlex data from 2023 and 2024, respectively with Web of Science data from 2024 and considering publications indexed in both data sources, Figure \@ref(fig:oalwos) shows that publications classified as articles in OpenAlex amount to `r formatC(sum(oal2324_wos_t[oal2324_wos_t$oal23_type == "ARTICLE", ]$n), big.mark = ",", format = "d")` (`r paste0(round(sum(oal2324_wos_t[oal2324_wos_t$oal23_type == "ARTICLE", ]$n) / sum(oal2324_wos_t$n) * 100, 2),"%")`) in 2023 and `r formatC(sum(oal2324_wos_t[oal2324_wos_t$oal24_type == "article", ]$n), big.mark = ",", format = "d")` (`r paste0(round(sum(oal2324_wos_t[oal2324_wos_t$oal24_type == "article", ]$n) / sum(oal2324_wos_t$n) * 100, 2),"%")`) in 2024 while in WoS publications with this document type amount to `r formatC(sum(oal2324_wos_t[oal2324_wos_t$wos_type == "Article", ]$n), big.mark = ",", format = "d")` (`r paste0(round(sum(oal2324_wos_t[oal2324_wos_t$wos_type == "Article", ]$n) / sum(oal2324_wos_t$n) * 100, 2),"%")`). 
...
Of the publications classified as articles in OpenAlex, the figures show that a total amount of 339,616 (Scopus) and 656,439 (WoS) publications are labelled as editorials or letters in Scopus and Web of Science, respectively.
