...
For this analysis, we gathered document types from WoS, Scopus and OpenAlex corresponding to publications indexed in all three databases. 
In addition, to align with the methods applied in our [preprint](https://arxiv.org/abs/2406.15154), we limited the data to the publication years 2012 to 2022 and only included items from journals.
To evaluate the recent progression of OpenAlex, we contrasted data from the OpenAlex July 2024 and August 2023 snapshots.
...
WoS data retrieval comprised the collections SCI, SSCI, AHCI, ISTP and ISSHP. 
We matched items between the databases by DOI after normalisation to lowercase.
We performed a pairwise comparison of OpenAlex with Scopus and WoS, respectively.
In sum, `r formatC(sum(oal2324_scp_t$n), big.mark = ",", format = "d")` publications were analysed for the OpenAlex and Scopus comparison and `r formatC(sum(oal2324_wos_t$n), big.mark = ",", format = "d")` publications for the OpenAlex and WoS comparison.

As in the preprint, we categorised works based on their document type information in two categories: research discourse and editorial discourse. Document types that did not belong to the assigned publication type, i.e. "journal", were not assigned to either of the two categories. 
A prevalent example of a document type not assigned to either category is [meeting abstracts](https://webofscience.help.clarivate.com/en-us/Content/document-types.html). 
Mapping tables that we used for reclassifying the document types can be found in the appendix of @haupka_2024. 
Additionally, publications with the document type "preprint", which was added to OpenAlex in [May 2024](https://groups.google.com/g/openalex-users/c/YujaIIjY02A), were also assigned to the research discourse category. 
...
Comparing OpenAlex data from 2023 and 2024 with Scopus data from 2024, Figure \@ref(fig:oalscp) shows, regarding publications indexed in both data sources, that OpenAlex classified `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$oal23_type == "ARTICLE", ]$noal)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal23_type == "ARTICLE", ]$noal)) / sum(unique(oal2324_scp_t$noal)) * 100, 2),"%")`) as articles in 2023 and `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "article", ]$noal)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "article", ]$noal)) / sum(unique(oal2324_scp_t$noal)) * 100, 2),"%")`) in 2024 while Scopus classified `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Article", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(oal2324_scp_t[oal2324_scp_t$scp_type == "Article", ]$n) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`) with this document type. 
Concerning publications classified as reviews, Figure \@ref(fig:oalscp) shows that within OpenAlex this document type is assigned to `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "review", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "review", ]$n24scp)) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`) in 2024 and within Scopus to `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Review", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Review", ]$n24scp)) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`). 
...
When comparing OpenAlex data from 2023 and 2024, respectively, with Web of Science data from 2024 and considering publications indexed in both data sources, Figure \@ref(fig:oalwos) shows that publications classified as articles amount to `r formatC(sum(unique(oal2324_wos_t[oal2324_wos_t$oal23_type == "ARTICLE", ]$noal)), big.mark = ",", format = "d")` (`r paste0(round(sum(unique(oal2324_wos_t[oal2324_wos_t$oal23_type == "ARTICLE", ]$noal)) / sum(unique(oal2324_wos_t$noal)) * 100, 2),"%")`) in OpenAlex for 2023 and `r formatC(sum(unique(oal2324_wos_t[oal2324_wos_t$oal24_type == "article", ]$noal)), big.mark = ",", format = "d")` (`r paste0(round(sum(unique(oal2324_wos_t[oal2324_wos_t$oal24_type == "article", ]$noal)) / sum(unique(oal2324_wos_t$noal)) * 100, 2),"%")`) in OpenAlex for 2024 while in WoS publications with this document type amount to `r formatC(sum(oal2324_wos_t[oal2324_wos_t$wos_type == "Article", "n24wos"][!duplicated(oal2324_wos_t[oal2324_wos_t$wos_type == "Article", c("oal24_type", "wos_type", "n24wos")]),]), big.mark = ",", format = "d")` (`r paste0(round(sum(oal2324_wos_t[oal2324_wos_t$wos_type == "Article", "n24wos"][!duplicated(oal2324_wos_t[oal2324_wos_t$wos_type == "Article", c("oal24_type", "wos_type", "n24wos")]),]) / sum(oal2324_wos_t$n24wos[!duplicated(oal2324_wos_t$n24wos)]) * 100, 2),"%")`). 
...
Regarding publications classified as reviews, Figure \@ref(fig:oalwos) shows, that within OpenAlex this document type is merely assigned to publications within the 2024 data set, these amount to `r formatC(sum(unique(oal2324_wos_t[oal2324_wos_t$oal24_type == "review", ]$n24wos)), big.mark = ",", format = "d")` (`r paste0(round(sum(unique(oal2324_wos_t[oal2324_wos_t$oal24_type == "review", ]$n24wos)) / sum(unique(oal2324_wos_t$n24wos)) * 100, 2),"%")`).
Within the WoS data set `r formatC(sum(unique(oal2324_wos_t[oal2324_wos_t$wos_type == "Review", ]$n24wos)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_wos_t[oal2324_wos_t$wos_type == "Review", ]$n24wos)) / sum(unique(oal2324_wos_t$n24wos)) * 100, 2),"%")`) have been classified as reviews. 
