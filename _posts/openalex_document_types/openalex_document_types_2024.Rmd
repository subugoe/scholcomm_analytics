...
Document types for this analysis were gathered from WoS, Scopus and OpenAlex. WoS data includes the collections SCI, SSCI, AHCI, ISTP and ISSHP. We only considered publications that occur in all of the three databases. In addition, to align with the methods applied in our [preprint](https://arxiv.org/abs/2406.15154), we restricted the data to the publication years 2012 to 2022 and only considered items from journals. 
To evaluate the recent progression of OpenAlex, we contrast the findings of the July 2024 snapshot with results obtained from OpenAlex in August 2023.
...
We matched items in the respective databases by DOI. DOIs were normalised to lowercase beforehand. In sum, we analysed ... .
We performed a pairwise comparison of OpenAlex with Scopus and WoS.

As in the preprint, we categorised works in two categories: research discourse and editorial discourse, based on their document type information. Document types that do not match the publication type, e.g. document type "book" and publication type "journal" were not assigned to one of the two categories. Another prevalent example of items that were not assigned are [meeting abstracts](https://webofscience.help.clarivate.com/en-us/Content/document-types.html). Mapping tables that we used for reclassifying the document types can be found in the appendix of @haupka_2024. In addition, publications with the document type "preprint", which was added to OpenAlex in [May 2024](https://groups.google.com/g/openalex-users/c/YujaIIjY02A), were also assigned to the research discourse category. 
...
Concerning publications classified as reviews, Figure \@ref(fig:oalscp) shows that within OpenAlex this document type is assigned to `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "review", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "review", ]$n24scp)) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`) in 2024 and within Scopus to `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Review", ]$n24scp)), big.mark = ",", format = "d")` (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Review", ]$n24scp)) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`). 
