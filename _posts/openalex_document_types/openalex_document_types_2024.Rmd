...
Following [preprint](https://arxiv.org/abs/2406.15154), we performed a pairwise comparison of journal publications indexed in OpenAlex with the Web of Science and Scopus published 2012 to 2022.
...
Web of Science data retrieval comprised the collections SCI, SSCI, AHCI, ISTP and ISSHP. 
We matched items between the databases by DOI after normalisation to lowercase.
Overall, `r formatC(sum(oal2324_scp_t$n), big.mark = ",", format = "d")` the intersection of OpenAlex and Scopus was `r formatC(sum(oal2324_scp_t$n), big.mark = ",", format = "d")`  and the intersection of OpenAlex and Web of Science comprised `r formatC(sum(oal2324_wos_t$n), big.mark = ",", format = "d")` records.
...
Before the introduction of the more nuanced set of document types, OpenAlex tagged
 `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$oal23_type == "ARTICLE", ]$noal)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal23_type == "ARTICLE", ]$noal)) / sum(unique(oal2324_scp_t$noal)) * 100, 2),"%")`) as articles, which reduced to 
`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "article", ]$noal)) / sum(unique(oal2324_scp_t$noal)) * 100, 2),"%")`). 
Scopus tagged `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Article", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(oal2324_scp_t[oal2324_scp_t$scp_type == "Article", ]$n) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`) as article. 
 OpenAlex assigned the tyoe review to `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "review", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$oal24_type == "review", ]$n24scp)) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`), whereas Scopus to `r formatC(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Review", ]$n24scp)), big.mark = ",", format = "d")` publications (`r paste0(round(sum(unique(oal2324_scp_t[oal2324_scp_t$scp_type == "Review", ]$n24scp)) / sum(unique(oal2324_scp_t$n24scp)) * 100, 2),"%")`). 
