...
It can be seen that the long tail of the least frequent 8 categories at the bottom of the table only makes up `r (1 - (articles_per_type_df %>% filter(prop > 1) %>% .$N_records %>% sum() / sum(articles_per_type_df$N_records))) * 100` % of all articles, which is why we will aggregate these evidence types in the category `Other` in the following.
