...
The `upset()` function generating the graph takes a bipartite matrix as input, where in our case one set is given by the evidence types and the other by the dois of the articles. The package has a built in function (`fromExpression`) for the conversion of an expression list to the required input format. We extracted the information on the frequency of the occurence of all combinations of evidence types as `.csv` files ([]() and []()) from our BigQuery tables using again SQL queries stored in seperate files ([evidence_overlap_08_12.sql](database/evidence_overlap_08_12.sql) and [evidence_overlap_13_18.sql](database/evidence_overlap_13_18.sql)) to be found in the source code repository. We then imported the resulting tables into R and converted them into the expression format which can be rendered by the `UpSetR` functions. To keep the resulting figure digestable, we only display the 20 combinations of the 7 most frequent types with the highest number of articles each.

<aside>
The total numbers for the displayed overlapping categories include also the amount of articles associated with intersections with sets, which are exlcuded here because of their low frequency. For example, the shown number of articles available via free pdf include articles with evidence via free pdf and via OAI-PMH title match since the latter is not considered seperately.
</aside>

```{r, layout = "l-page", fig.cap="Most frequent combinations of evidence types."}
library(UpSetR)
#fetch data for upset graph
evidence_categories_upset_08_12 <- readr::read_csv("~/Downloads/results-20190427-172458.csv")
evidence_categories_upset_13_18 <- readr::read_csv("~/Downloads/results-20190427-172458.csv")
