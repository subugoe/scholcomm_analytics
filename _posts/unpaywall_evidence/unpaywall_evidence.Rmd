...
plot_b <-
  ggplot(host_type_df, aes(x = year, y = number_of_articles, text = paste0("Publication year: ", year(year)))) +
  geom_bar(
    data = all_articles,
...
It can be seen that the long tail of the least frequent 8 categories at the bottom of the table with proportions smaller than 1 % of all articles only makes up `r (1 - (articles_per_type_df %>% filter(prop > 1) %>% .$N_records %>% sum() / sum(articles_per_type_df$N_records))) * 100` % of all articles in total, which is why we will aggregate these evidence types in the category `Other` in the following.
...
evidence_grouped_df %>%
  ggplot(aes(year, number_of_articles, fill = is_best, text = paste0("Publication year: ", year(year)))) +
  geom_bar(stat = "identity") +
...
The figure shows that Unpaywall found most open access full-text on publishers' websites (`r filter(host_type_intersect, grepl("Publisher", host, ignore.case = TRUE, fixed = FALSE)) %>% .$number_of_articles %>% sum() / host_type_intersect %>% .$number_of_articles %>% sum() * 100`%).  `r filter(host_type_intersect, host == "Publisher only") %>% .$number_of_articles %>% sum() / host_type_intersect %>% .$number_of_articles %>% sum() * 100` % of all open access full-texts was not archived in a repository. The overlap of open access provided by both routes also deserves attention: a proportion of around `r filter(host_type_intersect, host == "Publisher & Repository") %>% .$number_of_articles %>% sum() / host_type_intersect %>% .$number_of_articles %>% sum() * 100` % of all open access articles was accessible from both publishers' websites and repositories.
...
We are very grateful for helpful feedback on this post and ongoing discussions on the usage of Unpaywall data for bibliometric analyses with our project partners in Bielefeld, Dr. Niels Taubert and Elham Iravani, the related project [OASE](https://www.wihoforschung.de/de/oase-1969.php), in particular Nicholas Fraser and Prof. Dr. Philipp Mayr-Schlegel, Dr. Stephan Stahlschmidt and Dr. Aliakbar Akbaritabar from the [DZHW](https://www.dzhw.eu/en/), as well as Dr. Daniel Bangert and Dr. Birgit Schmidt from the [SUB Göttingen](https://www.sub.uni-goettingen.de/en/).
