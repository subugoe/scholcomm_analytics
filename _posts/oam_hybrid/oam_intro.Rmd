...
However, library consortia often lack data, which allow for measuring the impact of transformative agreements in terms of open access uptake across publisher portfolios and institutions over time. Here, we present [hoaddata](https://subugoe.github.io/hoaddata/), an R data package, in which openly available journal-level data about nationwide transformative agreements in Germany are combined with article-level open access status information and country affiliations. 

R packages like hoaddata are a standardised way to organise, document, test and publicly share a data analytics worklfow [@Marwick_2018]. In our specific case, hoaddata not only contains datasets about hybrid open access, but also code used to compile them by interfacing data dumps of the open scholarly data sources Crossref and OpenAlex, which [we store on Google Big Query](https://subugoe.github.io/scholcomm_analytics/data.html). 

The main purpose of hoaddata is to ship data for monitoring dashboards about the progress of nationwide transformative agreements, which we currently develop in the [HOAD project with the support of the Deutsche Forschungsgemeinschaft](https://gepris.dfg.de/gepris/projekt/416115939). The package is automatically build with GitHub Actions. Each merge event into the main branch triggers a data update. Data changes will be incorporated in the package and tracked with Git. This makes it possible to reproduce different version of the data contained in hoaddata. 

In this blog post, we describe the data analytics workflow behind hoaddata. To demonstrate its potential, we will use this package to illustrate the current state of the transition of hybrid journals to fully open access relative to those journals, wich are included in consortial transformative agreements in Germany. 


## Data and methods

hoaddata focuses on nationwide transformative agreements in Germany. As a first step, we draw on @VTQXLM_2021 to obtain a list of journals under these agreements, which is used by the German Open Access Monitor (OAM). 
We merged all journals into a single data file and enriched it with ISSN variants. 
Because there seems to be no direct data exchange between ISSN agencies and Crossref, we furthermore matched the OAM journal list with Crossref's title list to link ISSN variants to journals. 

After obtaining a list of hybrid journals linked to transformative agreements, we determined the article volume by journal and year using Crossref. 
Open access status information was derived from Creative Commons licenses URLs provided by Crossref, which were mapped to the different license versions like CC BY. 
...
A first author is often considered as [lead author](https://en.wikipedia.org/wiki/Lead_author) who has usually undertaken most of the research presented in the article, although author roles can vary across disciplines.
...
Note that DEAL has reached no agreement with Elsevier so far.
...
Interestingly, Germany's open access share in 2021 is in many cases below 80%, suggesting that not all authors made use of open access options, or were not eligible to publish open access, likely because their institution was not part of a consortium or the article type was not covered.
